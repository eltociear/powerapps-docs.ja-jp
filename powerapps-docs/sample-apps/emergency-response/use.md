...
ms.author: pankar
ms.reviewer: tapanm
ms.openlocfilehash: aa85ed9d96eb2ae33784bb79cd8628c5bc47710d
ms.sourcegitcommit: 997384ebd01bda4f456f08689e024fb8ca5bf057
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 06/15/2020
ms.locfileid: "3455068"
---
...
病院運営アプリを使用して、病院管理のために病院や施設固有のデータを収集します。
...
![COVID-19 の統計情報](media/use/covid19-stats.png)
...
データを送信した後、**COVID-19 統計情報** アプリに戻り、**他を追跡** ボタンを使用して別のレコードを作成することもできます。 **ホーム** を選択し、**病院の緊急対応アプリ** に戻ります。
...
データを送信した後、**備品** アプリに戻り、**他を追跡** ボタンを使用して別のレコードを作成することもできます。 **ホーム** を選択し、**病院の緊急対応アプリ** に戻ります。
...
データを送信した後、**スタッフ** アプリに戻り、**他を追跡** ボタンを使用して別のレコードを作成することもできます。 **ホーム** を選択し、**病院の緊急対応アプリ** に戻ります。
...
**ダッシュボード** アプリを使用したインサイトと意思決定のための Power BI ダッシュボードを表示します。 このオプションを選択すると、Power BIモバイル デバイス ブラウザーのダッシュボードが開きます。

ダッシュボードの詳細については、[Power BI ダッシュボードを見る](configure-data-reporting.md#view-power-bi-dashboard) をご覧ください。
...
- [ベッドのキャパシティ](#bed-capacity---cdc-reporting)
    <br> ICU、重症治療、増床キャパシティなどのベッド情報を収集します。
...
**新型コロナウイルスの患者への影響** アプリを使用して、新型コロナウイルス疑い、発症数、救急部門、オーバーフロー情報などの確認情報を送信します。 このアプリを使用してデータを送信した後、モデル駆動型アプリを使用して、CDC Webサイトへのアップロードなどの政府報告用のデータをダウンロードして保存できます。
...
IT 管理者は、Power Apps 用のモデル駆動型アプリを使用して、 **医療従事者のスタッフ配置** アプリの項目の一覧を追加または更新できます。 詳細については、[構成ガイド](deploy-configure.md) を参照してください。
...
| 重症治療ベッド (AIIR (空気感染隔離室) 以外) | 現在占有されている成人重症治療ベッド数 (非 AIIR (空気感染隔離室) 以外)
...
| **オプション名**                                               | **説明**                                                                       |
|---------------------------------------------------------------|---------------------------------------------------------------------------------------|
| 設備は許可ベッドの全キャパシティまでスタッフが配置されていますか?    | はい/いいえ。 答えが "いいえ" の場合、次のオプションから 1 つ以上の理由を選択できます。 <br> - スタッフ <br> - スペース <br> - 個人保護具 (PPE) <br> - 備品 <br> - 小規模な患者数  |
| 許可ベッド数を超えて急増できますか?              | はい/いいえ。 答えが "いいえ" の場合、次のオプションから 1 つ以上の理由を選択できます。 <br> - スタッフ <br> - スペース <br> - 個人保護具 (PPE) <br> - 備品 <br> - 小規模な患者数  |

### <a name="supply-planning---cdc-reporting"></a>消耗品の計画 - CDC 報告
...
- **人工呼吸器ディスポーザブル製品の使用/再利用**: 人工呼吸器消耗品の長時間使用または再利用の定義については、[希少なリソースの管理と治療の危機基準の概要](https://nwhrn.org/wp-content/uploads/2018/10/Scarce_Resource_Management_and_Crisis_Standards_of_Care_Overview_and_Materials-2020-04-10.pdf) ドキュメントを参照してください。
    <table>
      <tr>
        <th>WA ヘルス</th>
...
- **人工呼吸器消耗品 (使い捨て)**: W*人工呼吸器消耗品 (使い捨て)* の定義については、[CDC NHSN (全米医療安全ネットワーク)](https://www.cdc.gov/nhsn/) の Web サイトを参照してください。
