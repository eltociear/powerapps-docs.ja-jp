---
title: Color 列挙体と ColorFade、ColorValue、および RGBA 関数 | Microsoft Docs
description: 構文と例を含む Power Apps の Color 列挙体と ColorFade、ColorValue、および RGBA 関数の参照情報
author: gregli-msft
manager: kvivek
...
ms.locfileid: "3305533"
---
# <a name="color-enumeration-and-colorfade-colorvalue-and-rgba-functions-in-power-apps"></a>Power Apps の Color 列挙体と ColorFade、ColorValue、および RGBA 関数

組み込みのカラー値を使用し、カスタム カラーを定義し、アルファ チャネルを使用します。

## <a name="description"></a>内容

**Color** 列挙体を使用すると、HTML のカスケード スタイル シート (CSS) で定義されている色に簡単にアクセスできます。 たとえば、**Color.Red** は純粋な赤色を返します。 これらの色のリストは、このトピックの最後にあります。

**ColorValue** 関数は、CSS の色文字列に基づいて色を返します。 文字列は次のいずれかの形式になります。

- **CSS 色の名前:** たとえば、**"RoxyBrown"** および **"OliveDrab"** があります。 これらの名前にはスペースは含まれません。 サポートされている色のリストは、このトピックの後半にあります。
- **6 桁の 16 進数値:** たとえば、**"#ffd700"** は、**ゴールド**と同じです。 文字列の形式は "#*rrggbb*" で、*rr* は 2 桁の 16 進数の赤い部分、*gg* は緑、*bb* は青です。
- **8 桁の 16 進数値:** たとえば、**"#ff7f5080"** は、50 % のアルファ チャネルを持つ**コーラル**と同じです。 文字列の形式は "#*rrggbbaa*" で、*rr*、*gg*、および *bb* は 6 桁の形式と同一です。 アルファ チャネルは *aa* で表されます。**00** は完全な透明を表し、**ff** は完全な不透明を表します。

**RGBA** 関数は、赤、緑、および青色成分に基づいて色を返します。 この関数には、前面に重ねられたコントロールの色を混ぜ合わせるために使用されるアルファ チャネルも含まれています。 アルファ チャネル は、完全に透明で見えない 0 または 0% から、完全に不透明でコントロール下のレイヤーをまったく見えないようにする 1 または 100% まで変化します。

**ColorFade** 関数は、色のより明るいバージョンまたは暗いバージョンを返します。 フェードの量は、色を黒になるまで完全に暗くする -1 から、色に影響を与えない 0 を経て、色を白になるまで完全に明るくする 1 まで変化します。

## <a name="alpha-channel"></a>アルファ チャネル

キャンバス アプリでは、コントロールを前面に重ねて、その背後にあるコントロールに対してコントロールの透明度を指定できます。 その結果、色がレイヤーを介してブレンドされます。 たとえば、次の図は、3 つの原色が 50% のアルファ設定でどのように混ざるかを示しています。

> [!div class="mx-imgBorder"]
> ![アルファ設定が 50% の 3 つの原色](media/function-colors/alpha-primary.png)

アルファ チャネルをサポートするファイル形式で画像をブレンドすることもできます。 たとえば、.jpeg ファイルはブレンドできませんが、.png ファイルはブレンドできます。 次の図は、前の例と同じ赤、緑、青の色を示していますが、赤い色は、50% のアルファ チャネルを持つ .pngファイルで (円ではなく) 波線として表示されます。

> [!div class="mx-imgBorder"]
> ![青と緑の円の前にある、アルファ設定が 50% の赤の波線](media/function-colors/alpha-image.png)

**Color** 列挙体を指定するか、色の名前または 6 桁の 16 進数値で **ColorValue** 数式を作成する場合、アルファ設定は 100% であり、完全に不透明です。

## <a name="syntax"></a>構文

**Color**.*ColorName*

- *ColorName* - 必須。  カスケード スタイル シート (CSS) の色の名前。 可能な列挙体のリストは、このトピックの最後に表示されます。

**ColorValue**( *CSSColor* )

- *CSSColor* - 必須。  カスケード スタイル シート (CSS) の色の定義。 **OliveDrab** などの名前、または **#6b8e23** や **#7fffd420** などの 16 進数値を指定できます。 16 進数値は、#*rrggbb* または #*rrggbbaa* のいずれかの形式をとることができます。
...
- *FadeAmount* - 必須。  -1 ～ 1 の範囲の数。 -1 は色を黒になるまで完全に暗くし、0 は色に影響を与えず、1 は色を白になるまで完全に明るくします。 また、-100% ～ 100% の割合を使用することもできます。

## <a name="built-in-colors"></a>組み込みの色

| Color 列挙体 | ColorValue | RGBA | 色の見本 |
| --- | --- | --- | --- |
